...
def on_press(key):
    """
    Function dedcated to handle key press events.
    """
    try:
        ...
        if key == keyboard.Key.shift:
            ...
            if key.char == "q":
                ...
                with open(
                    "recorded_events.csv", "w", newline="", encoding="utf-8"
                ) as file:
...
def on_click(x_pos, y_pos, button, pressed):
    """
    Function dedicated to handle mouse click events.
    """
    if pressed:
        # Add mouse click event to the recorded events list
        recorded_events.append(
            ("mouse_click", (x_pos, y_pos, button), time.time() - current_time)
        )
...
with open("recorded_events.csv", "r", encoding="utf-8") as fr:
    ...
    for row in reader:
        ...
        if row_type == "key_press":
            ...
            print(f"Mouse clicked at: {data[2]}, {data[3]}")
            print(f"with {data[4]} button, and it took {data[0]} seconds to click it")
